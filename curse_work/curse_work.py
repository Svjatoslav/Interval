...
if __name__ == "__main__":
    ...
    plt.fill_between(y1=y1_1, y2=y1_2, x=x, color="cyan", alpha=0.2, label=r"$x_1+x_2 = [2.7,4.1]$")
    y2 = x
    plt.plot(x, y2, color="orange")
    y2_2 = 1.3 * x
    plt.plot(x, y2_2, color="orange", label=r'$\frac{x_1}{x_2} = [1, 1.3]$')
    plt.grid()
    plt.xlim(-2, 6)
    plt.ylim(-2, 6)
    plt.legend()
    ...
    plt.plot(x, y2, color="orange", label=r'$\frac{x_1}{x_2} = [1, 1.3]$')
    plt.plot(x, y2_2, color="orange")
    plt.fill_between(y1=y2, y2=y2_2, x=x, color="yellow", alpha=0.2)
    ...
    iveRect = plt.Rectangle((X_k[0][0].a, X_k[0][1].a), one-0.3, two+0.05, edgecolor='black', facecolor='none', linewidth=2,
                            label=r"$X^{(0)}$")
    ...
    plt.plot(x, y1_1, color="blue")
    plt.plot(x, y1_2, color="blue")
    plt.fill_between(y1=y1_1, y2=y1_2, x=x, color="cyan", alpha=0.2, label=r"$[0.7,0.9]x_1+[1.1, 1.2]x_2 = [2.7,4.1]$")
    plt.plot(x, y2_2, color="orange", label=r'$\frac{x_1}{x_2} = [1, 1.3]$')
    plt.plot(x, y2, color="orange", label=r'$\frac{x_1}{x_2} = [1, 1.3]$')
    plt.fill_between(y1=y2, y2=y2_2, x=x, color="yellow", alpha=0.2)
    print(len(X_k))
    for i in range(0, len(X_k)):
        ...
        print('FFFFFFFFFFFFFFFFFFFFFf', one, two, mid1, mid2)
        iveRect = plt.Rectangle((X_k[i][0].a, X_k[i][1].a), one-0.3, two+0.05, edgecolor='black', facecolor='none',
                                linewidth=1)
        plt.gca().add_patch(iveRect)
    plt.grid()
    plt.xlim(0.5, 3)
